...
# ---------------- CONFIG ----------------
DATA_DIR = r"D:\pdasSRD\Speech-risk-detection\ai\spectrograms"
WAV_DIR = r"D:\pdasSRD\Speech-risk-detection\ai\raw_data"
BATCH_SIZE = 8
EPOCHS = 20
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# --- DATABASE CONFIG (Public IP Method) ---
DB_HOST = "34.171.37.203"
DB_PASS = "qz*,2zZQ=4<6v4jY"
DB_USER = "postgres"
DB_NAME = "speaktrum_db"
DB_PORT = "5432"  # Changed for Postgres

# Encode the password so the special characters don't fuck up the URL
safe_pass = urllib.parse.quote_plus(DB_PASS)

# I changed it to postgresql+psycopg2
DB_URL = (
...
def extract_clinical_metrics(wav_path):
    try:
        ...
        hnr = call(
            call(sound, "To Harmonicity (cc)", 0.01, 75, 0.1, 1.0),
            "Get mean",
            0,
            0,
        )
        return {
            "jitter": round(jitter * 100, 4),
            "shimmer": round(shimmer * 100, 4),
            "hnr": round(hnr, 2),
        }
    except:
        ...
        for root, _, files in os.walk(wav_root):
            for file in files:
                if file.lower().endswith(".wav"):
                    wav_map[os.path.splitext(file)[0].upper().strip()] = (
...
if __name__ == "__main__":
    ...
    if len(full_dataset) == 0:
        ...
        print("🚀 Training for SpeakTrum Risk Detection...")
        for epoch in range(EPOCHS):
            ...
            scheduler.step()
            acc = 100 * t_corr / t_total
            print(
                f"Epoch {epoch+1} -> Loss: {t_loss/len(train_loader):.4f} | "
                f"Train Acc: {acc:.2f}%"
            )
        ...
        db_entries = []
        model.eval()
        print("\n📊 Calculating risk metrics and biomarkers...")
        with torch.no_grad():
            for i in range(len(full_dataset)):
                ...
                risk = (
                    "High"
                    if prob_pd > 0.8
                    else "Medium" if prob_pd > 0.4 else "Low"
                )
                clinical = extract_clinical_metrics(wav_path)
                db_entries.append(
                    {
                        "Patient_UUID": str(uuid.uuid4())[:8],
                        "Diagnosis_Timestamp": datetime.now(),  # <-- ADD THIS LINE
        ...
        # --- DATABASE EXPORT (Automatic Table Creation) ---
        try:
            results_df = pd.DataFrame(db_entries)

            # Postgres handles SSL args differently
            engine = create_engine(
                DB_URL,
                connect_args={"sslmode": "require"},
            )

            # This creates the table 'parkinsons_results' automatically
            results_df.to_sql(
                "parkinsons_results", engine, if_exists="append", index=False
            )

            print(
                f"✅ Successfully pushed {len(results_df)} records to "
                "SpeakTrum Cloud SQL (PostgreSQL)."
            )
